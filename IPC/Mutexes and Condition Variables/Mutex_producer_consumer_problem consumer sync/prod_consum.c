...
*                         between the multiple producer threads. Consumer thread
...
/******************************************************************************
* Function      : consume_wait
* Description   : consumer wait function.
*
* Parameters    : 
*   int i       : buffer index
* Return value  : void 
...
{
    /* Wait until the producers generate the ith item */
    for( ; ; )
    {
        if(pthread_mutex_lock(&shared.mutex) != 0)
        {
...
void *consume(void *arg)
{
    ...
    for(i = 0; i < nitems; i++)
    {
        /* Wait before fetching next item */
        consume_wait(i);
        if(shared.buff[i] != i)
            printf("buff[%d] = %d\n", i, shared.buff[i]);
    }
